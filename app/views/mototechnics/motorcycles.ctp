<?php  $this->Html->addCrumb('Мототехника', '');
			 if (isset($thiscrumb)) $link = '/mototechnics/motorcycles/'; else $link='';
			 $this->Html->addCrumb('Мотоциклы', $link); ?>
<h1>Мотоциклы</h1>
<div class="ca-text">
<div class="companies-menu">
	...
	<?php 
		foreach ($companies as $comp) {
			echo "<a href='/mototechnics/motorcycles/".$comp['mototechnics']['company']."' class='company ".$comp['mototechnics']['company']."'></a>";
	} ?>
</div>
<div class="strip"></div>
	<?php 
	  $lastcomp = '';
		$kol=0; 
		$cur=0;
		$size = count($motorcycles);
		foreach ($motorcycles as $key=>$moto) {
		  $cur++;
			//вывод ссылки на все модели
			if (!isset($thiscrumb)&&($kol>3)&&  
				($lastcomp != $moto['Mototechnic']['company'] || ($size == $cur))) {
						echo "<a href='/mototechnics/motorcycles/".$lastcomp."' class='show-more'>Показать все модели </a><br>";
			}
