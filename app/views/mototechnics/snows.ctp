<?php  $this->Html->addCrumb('Мототехника', ''); 
       if (isset($thiscrumb)) $link = '/mototechnics/snows/'; else $link='';
       $this->Html->addCrumb('Снегоходы', $link);?>
<h1>Снегоходы</h1>
<div class="ca-text">
<div class="companies-menu">
	<?php 
		foreach ($companies as $comp) {
			echo "<a href='/mototechnics/snows/".$comp['mototechnics']['company']."' class='company ".$comp['mototechnics']['company']."'></a>";
	} ?>
</div>
<div class="strip"></div>
	<?php 
	  $lastcomp = '';
		...
		$cur=0;
		$size = count($snows);
		foreach ($snows as $key=>$moto) {
		  $cur++;
			//вывод ссылки на все модели
			if (!isset($thiscrumb)&&($kol>3)&&  
				($lastcomp != $moto['Mototechnic']['company'] || ($size == $cur))) {
						echo "<a href='/mototechnics/snows/".$lastcomp."' class='show-more'>Показать все модели </a><br>";
			}
