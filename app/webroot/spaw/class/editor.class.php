  ...
  function getHtml()
  {
    ...
    if (SpawAgent::getAgent() != SPAW_AGENT_UNSUPPORTED)
    {
      ...
      foreach($this->pages as $pname => $page)
      {
        ...
        $pagetpl .= '<iframe name="'.$pname.'_rEdit" id="'.$pname.'_rEdit" style="width: 100%; height: '.$this->height.'; display: '.(($this->name == $pname)?'inline':'none').';" frameborder="no" src="/empty/empty.html?'.microtime().'"></iframe>';
